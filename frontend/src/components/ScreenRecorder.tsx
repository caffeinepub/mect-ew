...
import { Mic, MicOff, Monitor, Square, Upload, Download, Trash2, Video, Loader2, Play } from 'lucide-react';
  ...
  const startRecording = async () => {
    try {
      ...
      if (err.name !== 'NotAllowedError') {
        setErrorMessage('Error al iniciar la grabación: ' + (err.message || 'Error desconocido'));
  ...
  return (
    <div className="bg-card border border-border rounded-lg p-6 space-y-5">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-lg font-semibold text-foreground">Grabador de Pantalla</h3>
      </div>
      ...
      {(recordingState === 'idle' || recordingState === 'stopped' || recordingState === 'error') && (
        <div className="space-y-4">
          <div className="space-y-1.5">
            <Label htmlFor="rec-title" className="text-sm font-medium text-foreground">
              Título del Video
            </Label>
            <Input
              id="rec-title"
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {recordingState === 'stopped' && (
          <>
            ...
            <Button onClick={handleDiscard} variant="ghost" className="gap-2 text-destructive hover:text-destructive">
